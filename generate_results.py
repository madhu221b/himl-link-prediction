...
def get_visibility_plot(hmm, hMM, B=0, no_human=False):
    ...
    visibility_dict = {key:value-visibility_dict[0] for key,value in visibility_dict.items()}
...
def get_whole_cent_dict(main_path):
    dict_file = main_path+"/centrality.pkl"
    cent_dict = {}
...
def time_vs_betn_seeds(hMM,hmm):
    colors = ["#D8A7B1","#2C92EC","#38A055","#189AB4"]
    ...
    for i, seed in enumerate(seeds):
        ...
        cent_dict = get_whole_cent_dict(seed_path)
        
        b_dict = {}
        for j, B in enumerate([50,75,100,200]):
            seed_path = os.path.join(human_path, "seed_"+str(seed),"B_{}/dim_64/DPAH".format(B))
            h_cent_dict = get_whole_cent_dict(seed_path)
        ...
        if i == 0:
           ...
           if i == len(seeds) -1:
              no_h_std = [np.round(np.std(value),5)/10 for _,value in no_human_dict.items()]
              no_human_dict = {key:(np.round(np.mean(value),5)) for key,value in no_human_dict.items()}
              
              h_std =  [np.round(np.std(val),5)/10 for B,sub_dict in human_dict.items() for T, val in sub_dict.items()]
              ...
              print("no humn dict", no_human_dict)
    ...
    for i, (b, sub_dict) in enumerate(human_dict.items()):
        # ax.errorbar(sub_dict.keys(), sub_dict.values(), label="AL, B={}".format(b), marker="o",color=colors[i],yerr = h_std[b])
       # ax.errorbar(sub_dict.keys(), sub_dict.values(), label="AL, B={}".format(b), marker="o",color=colors[i],yerr = h_std[i])
        break
    #     dict_path = os.path.join(human_path,folder_path,"DPAH","_hMM{}_hmm{}.pkl".format(hMM,hmm))
    #     with open(dict_path, 'rb') as f:                
    #         centrality_dict = pkl.load(f)
...
def time_vs_visibility(hMM,hmm):
    ...
    for i, B in enumerate([75,100,200]):          
